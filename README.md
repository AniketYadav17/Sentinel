...
- Retrieval put on trial (2026-08): CONC 3 is 86 chunks, about 21K tokens, which fits in one prompt several times over, so the retrieval layer had never been shown to earn its keep at this scale. The control arm ran: the same judge, the same claims, the whole corpus in the prompt instead of dense top-5. It scored 1.000 against dense's 0.971, fixing the single error both provider families made. The pre-committed conclusion, written before the run so it could not be softened after, is that retrieval is a scaling investment for the Handbook-scale target and currently an accuracy tax at 86 chunks. The margin is one claim out of 34, which is 2.9 points, and it is quoted as a one-claim result rather than a proven improvement. The same phase added cost and latency instrumentation, so the other half of the trade is now measured too: the full-corpus arm costs about 7.8 times the input spend for that one claim. Five predictions were registered before the run and four sub-claims missed, including the headline one, with the scorecard in [`evals/README.md`](evals/README.md).
...
- **One replica, one reviewer.** The queue is SQLite on a mounted share, with a single lock around the graph. That is the right size for this project and would not be for a real deployment. Journalling is set to `DELETE` rather than the library default of WAL, because WAL needs a shared-memory file that SMB shares do not reliably provide.
